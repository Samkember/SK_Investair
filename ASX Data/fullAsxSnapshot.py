...
def fetch_stock_summary(ticker, company_name=None):
    while True:
        # If rate limit is active globally, wait
        ...
        try:
            ...
            if "Too Many Requests" in msg or "rate limit" in msg.lower():
                ...
                # ⏎ Loop again and retry this same ticker
                continue
            else:
                with print_lock:
...
def run_batch(asx_rows, summaries):
...
def fetch_all_asx_summaries_threaded():
    ...
    all_summaries = []
    failed_df = run_batch(base_df, all_summaries)
    ...
    while not failed_df.empty and retry_round <= MAX_RETRIES:
        ...
        failed_df = run_batch(failed_df, all_summaries)
        retry_round += 1
...
if __name__ == "__main__":
    df_all_asx = fetch_all_asx_summaries_threaded()
