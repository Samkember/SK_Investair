...
if __name__ == "__main__":
    ...
    for i in range(0, len(valid_tickers), 50):  # Update the step to 50
        ...
        if substantial_all:
            pd.concat(substantial_all, ignore_index=True).to_csv(f"/Users/samkember/Documents/SK_Investair/Small Cap Funds/ForTim080525/checkpoint_substantial_{i+50}.csv", index=False)
        if buying_all:
            pd.concat(buying_all, ignore_index=True).to_csv(f"/Users/samkember/Documents/SK_Investair/Small Cap Funds/ForTim080525/checkpoint_buying_{i+50}.csv", index=False)
        if selling_all:
            pd.concat(selling_all, ignore_index=True).to_csv(f"/Users/samkember/Documents/SK_Investair/Small Cap Funds/ForTim080525/checkpoint_selling_{i+50}.csv", index=False)

        print(f"💾 Checkpoint saved after {i+50} tickers.")

    # Final save
    pd.concat(substantial_all, ignore_index=True).to_csv("/Users/samkember/Documents/SK_Investair/Small Cap Funds/ForTim080525/all_substantial_shareholders.csv", index=False)
    pd.concat(buying_all, ignore_index=True).to_csv("/Users/samkember/Documents/SK_Investair/Small Cap Funds/ForTim080525/all_shareholders_buying.csv", index=False)
    pd.concat(selling_all, ignore_index=True).to_csv("/Users/samkember/Documents/SK_Investair/Small Cap Funds/ForTim080525/all_shareholders_selling.csv", index=False)
