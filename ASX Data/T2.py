import pandas as pd
import cloudscraper
from datetime import datetime
from dateutil.relativedelta import relativedelta

# === CONFIG ===
APP_ID = "af5f4d73c1a54a33"
OUTPUT_CSV = "ASX_Historical_Returns.csv"

# === FETCH TICKER LIST FROM ASX ===
def get_asx_tickers():
    url = "https://www.asx.com.au/asx/research/ASXListedCompanies.csv"
    df = pd.read_csv(url, skiprows=1)
    df = df[["ASX code", "Company name"]].dropna()
    df.columns = ["Ticker", "Company"]
    df["Ticker"] = df["Ticker"].str.strip().str.upper()
    return df

# === FETCH HISTORICAL RETURNS FOR A TICKER ===
def fetch_quoteapi_ticks_all_returns(ticker: str, app_id: str = APP_ID):
    scraper = cloudscraper.create_scraper()
    url = f"https://quoteapi.com/api/v5/symbols/{ticker.lower()}.asx/ticks"
    headers = {
        "accept": "application/json",
        "sec-ch-ua": '"Google Chrome";v="123", "Not:A-Brand";v="8", "Chromium";v="123"',
        "sec-ch-ua-mobile": "?0",
        "sec-ch-ua-platform": "\"Windows\"",
        "referer": f"https://www.marketindex.com.au/asx/{ticker.lower()}?src=search-all"
    }

    range_options = ["20y", "5y", "1y", "6m", "3m"]

    for range_ in range_options:
        try:
            params = {
                "appID": app_id,
                "adjustment": "capital",
                "fields": "dc",
                "range": range_
            }

            response = scraper.get(url, headers=headers, params=params, timeout=10)
            response.raise_for_status()
            data = response.json()

            ticks = data.get("ticks", {})
            dates = ticks.get("date", [])
            closes = ticks.get("close", [])

            if not dates or not closes or len(dates) != len(closes):
                print(f"⚠️ {ticker}: No usable ticks in range {range_}")
                continue

            print(f"✅ {ticker}: Using range {range_}")
            df = pd.DataFrame({"Date": dates, "Price": closes})
            df["Date"] = pd.to_datetime(df["Date"])
            df.set_index("Date", inplace=True)
            df.sort_index(inplace=True)
            df = df[df["Price"].notnull()]  # remove nulls

            if df.empty:
                continue

            latest_price = df["Price"].iloc[-1]
            today = df.index[-1]

            offsets = {
                "1 Month Return (%)": today - relativedelta(months=1),
                "3 Month Return (%)": today - relativedelta(months=3),
                "6 Month Return (%)": today - relativedelta(months=6),
                "12 Month Return (%)": today - relativedelta(months=12),
                "3 Year Return (%)": today - relativedelta(years=3),
                "5 Year Return (%)": today - relativedelta(years=5),
            }

            returns = {"Ticker": ticker, "Latest Price": latest_price}
            for label, target_date in offsets.items():
                valid_dates = df.index[df.index <= target_date]
                if not valid_dates.empty:
                    closest_date = valid_dates.max()
                    past_price = df.loc[closest_date, "Price"]
                    returns[label] = round(((latest_price - past_price) / past_price) * 100, 2)
                else:
                    returns[label] = None

            return returns

        except Exception as e:
            print(f"⚠️ {ticker}: Failed with range {range_}: {e}")
            continue

    print(f"❌ {ticker}: No valid range found")
    return {"Ticker": ticker, "Latest Price": None,
            "1 Month Return (%)": None, "3 Month Return (%)": None,
            "6 Month Return (%)": None, "12 Month Return (%)": None,
            "3 Year Return (%)": None, "5 Year Return (%)": None}

# === MAIN EXECUTION ===
def main():
    ticker_df = get_asx_tickers()
    results = []

    for i, row in ticker_df.iterrows():
        ticker = row["Ticker"]
        try:
            returns = fetch_quoteapi_ticks_all_returns(ticker)
            results.append(returns)
        except Exception as e:
            print(f"🚫 Skipping {ticker}: {e}")

        # Save every 5 tickers
        if (i + 1) % 5 == 0:
            df_partial = pd.DataFrame(results)
            df_partial.to_csv(OUTPUT_CSV, index=False)
            print(f"💾 Saved progress at {i + 1} tickers to {OUTPUT_CSV}")

    # Final save
    df_results = pd.DataFrame(results)
    df_results.to_csv(OUTPUT_CSV, index=False)
    print(f"\n✅ Final data saved to {OUTPUT_CSV}")

if __name__ == "__main__":
    main()
