...
# === TABLE NAME (ENV OVERRIDABLE) ===
TRIGGER_TABLE_NAME = "market_triggers_2"

# === SIGNAL DETECTION FUNCTIONS ===
def annotate_signal_context(df, signal_rows):
    signal_rows = signal_rows.merge(
        df[["snapshot_date", "close_price", "volume"]], on="snapshot_date", how="left"
    )
...
def detect_increasing_volume_price_ema_signals(data: pd.DataFrame, ticker: str, fast_span=5, slow_span=20):
    df = data[data["ticker"] == ticker].copy().sort_values("snapshot_date")
    ...
    signals_df["reason"] = signals_df.apply(
        lambda x: f"Price EMA fast > slow by {x['price_diff']:.2f}; Volume EMA fast > slow by {int(x['volume_diff'])}",
        axis=1
    )
...
def detect_price_reversal_signals(data, ticker, price_trend_window=5, min_trend=2,
                                  fast_span=5, slow_span=20, zscore_threshold=2.0, zscore_window=10):
    df = data[data["ticker"] == ticker].copy().sort_values("snapshot_date").reset_index(drop=True)
    df["snapshot_date"] = pd.to_datetime(df["snapshot_date"])
    df["close_ema_fast"] = df["close_price"].ewm(span=fast_span, adjust=False).mean()
    df["close_ema_slow"] = df["close_price"].ewm(span=slow_span, adjust=False).mean()

    df["trend"] = np.where(df["close_ema_fast"] > df["close_ema_slow"], 1,
                   np.where(df["close_ema_fast"] < df["close_ema_slow"], -1, 0))
    df["volume_pct_change"] = df["volume"].pct_change().replace([np.inf, -np.inf], np.nan)
...
def detect_breakout_signals(data, ticker, window=20, volume_zscore_threshold=2.0, zscore_window=10):
    ...
    df.loc[(df["close_price"] > df["range_high"]) & (df["vol_zscore"] > volume_zscore_threshold), "reason"] = \
        df.apply(lambda row: f"Price {row['close_price']:.2f} broke above {row['range_high']:.2f}, Volume Z-score {row['vol_zscore']:.2f}", axis=1)
    df.loc[(df["close_price"] < df["range_low"]) & (df["vol_zscore"] > volume_zscore_threshold), "signal_type"] = "bear_breakout"
    df.loc[(df["close_price"] < df["range_low"]) & (df["vol_zscore"] > volume_zscore_threshold), "reason"] = \
        df.apply(lambda row: f"Price {row['close_price']:.2f} broke below {row['range_low']:.2f}, Volume Z-score {row['vol_zscore']:.2f}", axis=1)
...
def process_ticker(ticker):
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                """
                SELECT snapshot_date, close_price, volume, ticker
                FROM asx_market_snapshot
                WHERE ticker = %s
        ...
        with engine.begin() as conn:
            for _, row in signals.iterrows():
                result = conn.execute(
                    text(
                        f"""
                        INSERT IGNORE INTO {TRIGGER_TABLE_NAME}
                            (signal_type, ticker, snapshot_date, reason, price_before, price_after, volume_pct_change)
                        VALUES
                            (:signal_type, :ticker, :snapshot_date, :reason, :price_before, :price_after, :volume_pct_change)
                        """
                    ),
...
if __name__ == "__main__":
    ...
    if failures:
        failed_codes = [f["ticker"] for f in failures]
        print(f"\nFAILED ({len(failed_codes)}): {failed_codes}")
