...
def detect_increasing_volume_price_ema_signals(data: pd.DataFrame, ticker: str, fast_span=5, slow_span=20):
    ...
    return signals_df[['signal_type', 'ticker', 'snapshot_date', 'reason']]
    ...
    try:
        ...
        with engine.begin() as conn:
            for _, row in signals.iterrows():
                result = conn.execute(
                    text("""
                    INSERT IGNORE INTO triggers3 (signal_type, ticker, snapshot_date, reason)
...
if __name__ == "__main__":
    ...
    with engine.begin() as conn:
        conn.execute(text("""
        CREATE TABLE IF NOT EXISTS triggers3 (
